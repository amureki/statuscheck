...
class ServiceAPI(BaseServiceAPI):
    ...
    def get_status(self) -> Status:
        heroku_summary = self._get_summary()
        return heroku_summary.status

    def get_summary(self) -> Summary:
        heroku_summary = self._get_summary()
        status = heroku_summary.status
        components = [
            Component(name=component.name, status=component.status,)
            for component in heroku_summary.components
        ]
        incidents = [
            Incident(
                id=incident.id,
            ...
            for incident in heroku_summary.incidents
        ]
